...
export default function VacancyCandidatesTableRows({
  ...
  if (state === 'loading') {
    return (
      <AdminTableLoadingRows cellWidths={['w-30', 'w-24', 'w-16', 'w-18', 'w-8', 'w-8', 'w-24']} />
    );
  }

  if (rows.length === 0) {
    return <AdminTableEmptyRow colSpan={7} message="No candidate submissions found." />;
  }
            ...
            <td className="px-2 py-2 text-center">
              <span
                className={`mx-auto inline-flex w-10 items-center justify-center 
                  rounded-(--radius-secondary) border px-2 py-0.5 
                  text-main-xs ${
                    row.commentsCount > 0
              ...
              <div className="flex w-full items-center gap-2">
                ...
                <div className="relative ml-auto min-w-34">
                  <select
                    ...
                    onChange={(event) =>
                      void onUpdateStatus(row.id, event.target.value as VacancySubmissionStatus)
                    }
