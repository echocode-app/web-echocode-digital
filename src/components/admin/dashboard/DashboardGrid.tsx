...
import DashboardSiteSliceChartsSection from '@/components/admin/dashboard/DashboardSiteSliceChartsSection';
import DashboardSiteSliceOverview from '@/components/admin/dashboard/DashboardSiteSliceOverview';
...
const AreaTrafficChart = dynamic(
  () => import('@/components/admin/dashboard/charts/AreaTrafficChart'),
  {
    ssr: false,
...
export default function DashboardGrid() {
  ...
  const [visibleExtraBatches, setVisibleExtraBatches] = useState(0);
  const trafficChartMobileMinWidthClass =
    period === 'month'
      ? 'min-w-[44rem] sm:min-w-[48rem] md:min-w-0'
      : 'min-w-[22rem] sm:min-w-[26rem] md:min-w-0';
  const canLoadMore = visibleExtraBatches < 2;
  ...
  return (
    <section className="min-w-0 space-y-4">
      ...
      <DashboardSiteSliceOverview period={period} enabled />
      ...
      {visibleExtraBatches >= 2 ? <DashboardSiteSliceChartsSection enabled /> : null}
      ...
      {canLoadMore ? (
        <div className="space-y-4">
          ...
          <button
            type="button"
            onClick={() => setVisibleExtraBatches((prev) => Math.min(prev + 1, 2))}
