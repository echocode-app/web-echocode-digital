...
export function useEmailSubmissionDetails(submissionId: string) {
  const [state, setState] = useState<LoadState>('loading');
  const [details, setDetails] = useState<EmailSubmissionDetailsItemDto | null>(null);
  ...
  useEffect(() => {
    ...
    fetchEmailSubmissionDetails(submissionId, controller.signal)
      .then((payload) => {
  ...
  const onAddComment = useCallback(async () => {
    ...
    try {
      const createdComment: ModerationCommentDto = await addEmailSubmissionComment({
